"""Robustness audit for Sleeve B (TSMOM).

Asks: is the headline 0.71 Sharpe a real edge, or fragile to parameter choice?

Sections:
  1. Baseline (sanity re-run)
  2. Walk-forward train/test split
  3. Parameter sensitivity grid (63 configs)
  4. Sub-period analysis (rolling 4-year windows)
  5. Leave-one-out asset drop
  6. Transaction-cost sensitivity
  7. Randomized-signal placebo (100 seeds) -- the canonical "is this just leverage?" test
"""
...
def section_baseline(monthly: pd.DataFrame) -> dict:
    ...
    print(
        ...
        f"({len(monthly)} months, {len(monthly.columns)} assets)"
    )
...
def section_walk_forward(monthly: pd.DataFrame) -> None:
    ...
    test = monthly.iloc[split - 12 :]  # 12m warmup for signal
...
def section_parameter_sensitivity(monthly: pd.DataFrame) -> None:
    print("\n=== 3. Parameter Sensitivity (63-config grid) ===")
    rows = []
    ...
    print(f"  % configs with Sharpe > 0.50:    {(df['sharpe'] > 0.5).mean():.1%}")
    ...
    print("\n  Sharpe averaged by lookback (averaged across vol/leverage):")
...
def section_sub_periods(monthly: pd.DataFrame) -> None:
    ...
    rows = []
    n = len(monthly)
    for start in range(0, n - period_months + 1, 24):  # step 2y for overlap
    ...
    print(
        f"\n  Sub-periods with Sharpe > 0:    "
        f"{(df['sharpe'] > 0).sum()}/{len(df)}"
    )
    print(
        f"  Sub-periods with Sharpe > 0.3:  "
        f"{(df['sharpe'] > 0.3).sum()}/{len(df)}"
    )
...
def section_leave_one_out(monthly: pd.DataFrame) -> None:
    ...
    print(f"  Baseline (all {len(monthly.columns)} assets) Sharpe: {base:.4f}\n")
    print(df.to_string(index=False))
    print(
        f"\n  Max single-asset Sharpe delta: "
        f"{df['delta_vs_base'].abs().max():.3f}  "
        f"=> {'NOT DOMINATED' if df['delta_vs_base'].abs().max() < 0.20 else 'CONCENTRATED'}"
    )
...
def section_cost_sensitivity(monthly: pd.DataFrame) -> None:
    ...
    df = pd.DataFrame(rows)
    print(df.to_string(index=False))
    breakeven = df[df["sharpe"] < 0]
    if breakeven.empty:
        print("  Sharpe stays positive even at 200bps per rebalance.")
    else:
        print(f"  Sharpe goes negative at cost >= {breakeven.iloc[0]['cost_bps']} bps.")
...
def section_placebo(monthly: pd.DataFrame, n_seeds: int = 100) -> None:
    ...
    for _ in range(n_seeds):
        ...
        cost = (
            position.diff().abs().mean(axis=1) * cfg.transaction_cost_bps / 10_000.0
        )
    ...
    if p_value < 0.05:
        ...
        verdict = "WEAK EDGE (real signal beats random at p<0.20)"
    else:
        verdict = "NO DETECTABLE EDGE (real signal indistinguishable from random)"
